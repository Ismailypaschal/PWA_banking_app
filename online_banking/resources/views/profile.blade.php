        ...
        <div class="flex flex-col items-center pt-2 pb-8 px-4">
            <div class="relative mb-5 group">
                <div
                    ...
                    @else
                        <img src="data:image/svg+xml;base64,"
                            alt="Default Picture" class="w-full h-full object-cover rounded-full" />
                    @endif
                </div>
                ...
                <form action="{{ route('user.update-profile') }}" method="POST" enctype="multipart/form-data"
                    ...
                    @csrf
                    <!-- Hidden file input -->
                    <input id="photo-upload" type="file" name="profile_photo" accept="image/*" class="hidden"
                        onchange="this.form.submit()" />

                    <!-- Edit button -->
                    <label for="photo-upload"
                        class="bg-primary hover:bg-blue-600 text-white rounded-full p-2
            ...
            <div class="flex items-center gap-2 mt-1">
                {{-- last 5 digits --}}
                @php
                    $last5 = substr($account->account_number, -5);
                @endphp
                <span class="text-[#637588] dark:text-[#92a4c9] text-sm font-medium">Premier
                    *****{{ $last5 }}</span>
            </div>
            <div
        ...
        <div class="px-4 flex flex-col gap-6 max-w-lg mx-auto w-full">
                        ...
                        <div
                            ...
                            <div class="space-y-1">
                                <label
                                    ...
                                    Number</label>
                                <p class="text-sm font-medium text-[#111418] dark:text-white">{{ $user->phone }}</p>
                            </div>
                            <div class="space-y-1">
                                <label
                                    class="text-[10px] font-bold uppercase tracking-wider text-[#637588] dark:text-[#92a4c9]">Residential
                                    Address
                                </label>
                                <p class="text-sm font-medium text-[#111418] dark:text-white">{{ $user->address }}</p>
                            </div>
                            <div class="space-y-1">
                                <label
                                    class="text-[10px] font-bold uppercase tracking-wider text-[#637588] dark:text-[#92a4c9]">Country
                                </label>
                                <p class="text-sm font-medium text-[#111418] dark:text-white">{{ $user->country }}</p>
                            </div>
                            <div class="space-y-1">
                                <label
                                    class="text-[10px] font-bold uppercase tracking-wider text-[#637588] dark:text-[#92a4c9]">City
                                </label>
                                <p class="text-sm font-medium text-[#111418] dark:text-white">{{ $user->city }}</p>
                            </div>
                            <!-- <button
                ...
                <div
                            ...
                            <div
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <input
                                            class="w-full bg-white dark:bg-[#1c2431] border-gray-200 dark:border-white/10 rounded-lg text-sm focus:ring-primary focus:border-primary"
                                            name="new_password" placeholder="••••••••" type="password" />
                                    </div>
                                </div>
                                <button type="submit"
                                    class="w-full py-2.5 bg-primary text-white text-sm font-bold rounded-lg">Update
                                    Password</button>
                            </div>
                        </form>
                    </details>
                    <details class="border-b border-gray-100 dark:border-white/5">
                        <summary
                            ...
                            <div class="flex-1 text-left">
                                <p class="text-base font-medium text-[#111418] dark:text-white">Change Security Pin</p>
                                <p class="text-[#637588] dark:text-[#92a4c9] text-xs mt-0.5">
                                </p>
                                {{-- <p class="text-[#637588] dark:text-[#92a4c9] text-xs mt-0.5"><%= user.passwordUpdatededAt ? 'Last changed on ' + user.passwordUpdatedAt.toDateString() : 'You have not changed your password yet' %>
                                </p> --}}
                        ...
                        <form method="POST" action="{{ route('update.security-pin') }}">
                            @csrf
                            <div
                                class="p-4 bg-gray-50/50 dark:bg-[#151c2a] border-t border-gray-100 dark:border-white/5 space-y-4">
                                <div class="space-y-3">
                                    <div class="space-y-1">
                                        ...
                                        <input
                                            class="w-full bg-white dark:bg-[#1c2431] border-gray-200 dark:border-white/10 rounded-lg text-sm focus:ring-primary focus:border-primary"
                                            name="old_pin" placeholder="••••" type="password" />
                                    </div>
                                    <div class="space-y-1">
                                        <label
                                            class="text-[10px] font-bold uppercase text-[#637588] dark:text-[#92a4c9]">New
                                        ...
                                        <input
                                            class="w-full bg-white dark:bg-[#1c2431] border-gray-200 dark:border-white/10 rounded-lg text-sm focus:ring-primary focus:border-primary"
                                            name="new_pin" placeholder="••••" type="password" />
                                    </div>
                                </div>
                                <button type="submit"
                                    class="w-full py-2.5 bg-primary text-white text-sm font-bold rounded-lg">Update
                    ...
                    <details class="border-b border-gray-100 dark:border-white/5">
                        <summary
                            ...
                            <div class="flex-1 text-left">
                                ...
                                <p class="text-[#637588] dark:text-[#92a4c9] text-xs mt-0.5">Enabled (SMS &amp; App)
                                </p>
                            </div>
                            <span
                                class="material-symbols-outlined text-gray-400 dark:text-gray-500 accordion-arrow transition-transform duration-200">expand_more</span>
            ...
            <form method="POST" action="{{ route('user.logout') }}">
                ...
                <button
                    ...
                    <span
                        class="material-symbols-outlined transition-transform group-hover:-translate-x-1">logout</span>
                    Log Out
                </button>
            </form>
