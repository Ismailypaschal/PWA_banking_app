...
@if (Auth::check() && !Auth::user()->hasVerifiedEmail())
    <div
        style="border:1px solid #d1d5db; background:#fff7ed; padding:16px; border-radius:6px; color:#92400e; max-width:720px; margin:12px auto;">
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:8px;">
            <form method="POST" action="{{ route('verification.send') }}" style="display:inline;">
                @csrf
                <button type="submit"
                    style="background:#0ea5a3; color:#fff; border:none; padding:8px 12px; border-radius:4px; cursor:pointer;">
