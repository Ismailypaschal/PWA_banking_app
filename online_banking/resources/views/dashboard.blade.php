...
<body class="bg-background-light dark:bg-background-dark text-[#111418] dark:text-white font-display overflow-x-hidden">
    <div class="relative flex h-full w-full flex-col min-h-screen">
        ...
        <div
            class="flex items-center px-4 pt-4 pb-2 justify-between sticky top-0 z-50 bg-background-light/90 dark:bg-background-dark/90 backdrop-blur-md">
            <div class="flex items-center gap-3">
                <div class="size-10 rounded-full bg-center bg-cover border-2 border-primary/20">
                    @if ($user->profile_photo)
                        <img class="rounded-full w-full h-full object-cover" src="{{ $user->profile_photo }}"
                            alt="Profile Picture">
                    @else
        ...
        <div class="flex flex-col gap-2 px-4 py-4">
            <div
                ...
                <div class="relative z-10">
                    ...
                    <button type="button"
                        class="text-blue-200 hover:text-white transition-colors absolute top-10 right-2" id="hide-btn">
                        <span class="material-symbols-outlined text-sm" id="icon">visibility</span>
                    </button>
        ...
        <div class="flex flex-col mt-6">
            <div class="flex items-center justify-between px-4 mb-3">
                ...
                <a href="{{ route('user.account-details') }}" class="text-primary text-sm font-bold">See All</a>
            </div>
        ...
        <div class="flex flex-col px-4 mt-6 pb-24">
            <div class="flex items-center justify-between mb-4">
                ...
                @if ($transactions->isNotEmpty())
                    <a href="{{ route('user.transactions') }}" class="mr-2 text-green-500 dark:text-green-500">View
                        All</a>
                @endif
            </div>
    ...
    <div id="successModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-50">
        ...
        <div class="bg-white dark:bg-[#1a2233] w-[90%] max-w-sm rounded-2xl p-6 shadow-2xl text-center">
            ...
            <p class="text-sm text-slate-600 dark:text-slate-300 mb-6" style="overflow-wrap:anywhere;">
                {{ session('success') }}
            </p>
    ...
    <script>
        // Fixed toggle visibility functionality for the dashboard
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (hideBtn) {
                hideBtn.addEventListener('click', () => {
                    if (!isHidden) {
                        // Store the original value in data attribute
                        amount.dataset.realValue = amount.textContent.trim();
                        // Hide the amount
                        amount.textContent = '•••••• USD';
                        icon.textContent = 'visibility_off';
                        isHidden = true;
                    } else {
                        // Show the amount again
                        amount.textContent = amount.dataset.realValue;
