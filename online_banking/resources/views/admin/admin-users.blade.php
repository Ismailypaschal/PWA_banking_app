<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8" />
    <meta content="width=device-width, initial-scale=1.0" name="viewport" />
    <title>Admin User Management</title>

    <link href="https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;700&amp;display=swap" rel="stylesheet" />
    <link
        href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:wght,FILL@100..700,0..1&amp;display=swap"
        rel="stylesheet" />

    <link rel="stylesheet" href="/output.css">
    <script src="/js/general-theme.js"></script>
</head>

<body class="bg-background-light dark:bg-background-dark min-h-screen text-slate-900 dark:text-white">
    <header
        class="sticky top-0 z-50 bg-background-light dark:bg-background-dark border-b border-slate-200 dark:border-slate-800">
        <div class="flex items-center p-4 pb-2 justify-between">
            <div id="backBtn" class="flex size-10 shrink-0 items-center justify-center cursor-pointer">
                <span class="material-symbols-outlined text-2xl">arrow_back_ios</span>
            </div>
            <h2 class="text-lg font-bold leading-tight tracking-tight flex-1 text-center">User Management</h2>
            <div class="flex w-10 items-center justify-end">
                <a href="{{ route('user.signup') }}" target="_blank"
                    class="flex items-center justify-center rounded-lg h-10 w-10 bg-primary text-white">
                    <span class="material-symbols-outlined text-xl">person_add</span>
                </a>
            </div>

        </div>

        <form method="GET" action="{{ route('admin.users') }}">
            {{-- search input --}}
            <div class="px-4 py-3">
                <label class="flex flex-col w-full">
                    <div
                        class="flex w-full items-stretch rounded-xl h-11 bg-slate-200/50 dark:bg-slate-800/50 border border-transparent focus-within:border-primary transition-all">
                        <div class="flex items-center justify-center pl-4 text-slate-500">
                            <span class="material-symbols-outlined text-xl">search</span>
                        </div>
                        <input type="text" name="search" value="{{ request('search') }}"
                            class="bg-transparent border-none focus:ring-0 flex-1 px-3 text-sm font-normal placeholder:text-slate-500"
                            placeholder="Search accounts or IDs..." />
                    </div>
                </label>
            </div>

            <div class="px-4 pb-4">
                <div class="flex h-11 items-center justify-center rounded-xl bg-slate-200/50 dark:bg-slate-800/50 p-1">
                    <label
                        class="flex cursor-pointer h-full grow items-center justify-center rounded-lg px-2 has-[:checked]:bg-white dark:has-[:checked]:bg-slate-700 has-[:checked]:shadow-sm transition-all text-slate-500 has-[:checked]:text-primary dark:has-[:checked]:text-white text-xs font-bold uppercase tracking-wider">
                        <span>All</span>
                        <input onchange="this.form.submit()" checked class="hidden" name="status" type="radio"
                            value="all" {{ request('status', 'all') == 'all' ? 'checked' : '' }} />
                    </label>
                    <label
                        class="flex cursor-pointer h-full grow items-center justify-center rounded-lg px-2 has-[:checked]:bg-white dark:has-[:checked]:bg-slate-700 has-[:checked]:shadow-sm transition-all text-slate-500 has-[:checked]:text-primary dark:has-[:checked]:text-white text-xs font-bold uppercase tracking-wider">
                        <span>Active</span>
                        <input onchange="this.form.submit()" class="hidden" name="status" type="radio" value="active"
                            {{ request('status') == 'active' ? 'checked' : '' }} />
                    </label>
                    <label
                        class="flex cursor-pointer h-full grow items-center justify-center rounded-lg px-2 has-[:checked]:bg-white dark:has-[:checked]:bg-slate-700 has-[:checked]:shadow-sm transition-all text-slate-500 has-[:checked]:text-primary dark:has-[:checked]:text-white text-xs font-bold uppercase tracking-wider">
                        <span>Inactive</span>
                        <input onchange="this.form.submit()" class="hidden" name="status" type="radio"
                            value="inactive" {{ request('status') == 'inactive' ? 'checked' : '' }} />
                    </label>
                    <label
                        class="flex cursor-pointer h-full grow items-center justify-center rounded-lg px-2 has-[:checked]:bg-white dark:has-[:checked]:bg-slate-700 has-[:checked]:shadow-sm transition-all text-slate-500 has-[:checked]:text-primary dark:has-[:checked]:text-white text-xs font-bold uppercase tracking-wider">
                        <span>Suspended</span>
                        <input onchange="this.form.submit()" class="hidden" name="status" type="radio"
                            value="suspended" {{ request('status') == 'suspended' ? 'checked' : '' }} />
                    </label>
                </div>
            </div>
        </form>
    </header>
    <main class="p-4 space-y-4 pb-24">
        <div class="flex items-center justify-between">
            <p class="text-xs font-bold uppercase text-slate-500 tracking-widest">User Accounts
                ({{ $userAllCount }})</p>
            <span class="material-symbols-outlined text-slate-400 text-lg">filter_list</span>
        </div>
        @foreach ($users as $user)
            <div
                class="bg-white dark:bg-slate-900 rounded-xl p-4 shadow-sm border border-slate-100 dark:border-slate-800">
                <div class="flex flex-col gap-3 mb-4">
                    <div class="flex justify-between items-start">
                        <div class="flex-1 min-w-0">
                            <p class="font-bold text-base truncate">{{ $user->first_name }} {{ $user->last_name }}</p>
                            <p class="text-slate-500 text-xs truncate">ACCOUNT NO:
                                {{ $user->accounts->account_number }}
                            </p>
                        </div>
                        @if ($user->status === 'active')
                            <div
                                class="bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-tighter">
                                {{ ucfirst($user->status) }}
                            </div>
                        @elseif($user->status === 'inactive')
                            <div
                                class="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-tighter">
                                {{ ucfirst($user->status) }}
                            </div>
                        @elseif($user->status === 'suspended')
                            <div
                                class="bg-black dark:bg-white text-white dark:text-black text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-tighter">
                                {{ ucfirst($user->status) }}
                            </div>
                        @endif
                    </div>
                    <div class="flex items-center gap-2 mt-2">
                        <a href="{{ route('admin.user.deposits', $user->id) }}"
                            class="flex flex-1 items-center justify-center py-2 px-3 rounded-lg bg-gray-600 dark:bg-white text-green-600 dark:text-green-400 text-xs font-bold hover:bg-green-100 dark:hover:bg-red-900/40 transition-colors">
                            <span class="material-symbols-outlined text-sm">arrow_downward</span> Deposit
                        </a>
                        <a href="{{ route('admin.user.transfers', $user->id) }}"
                            class="flex flex-1 items-center justify-center py-2 px-3 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-xs font-bold hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
                            <span class="material-symbols-outlined text-sm">swap_horiz</span> Transfer
                        </a>
                    </div>
                    <div class="flex items-center justify-between bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg">
                        <span class="text-xs text-slate-500 font-medium">Account Balance</span>
                        <span
                            class="text-sm font-bold text-slate-900 dark:text-white">${{ $user->accounts->balance }}</span>
                    </div>
                </div>
                {{-- flash messages --}}
                @if (session('success'))
                    <div class="mb-4 p-3 bg-green-100 text-green-500 rounded-lg text-center">
                        {{ session('success') }}
                    </div>
                @endif
                @if (session('error'))
                    <div class="mb-4 p-3 bg-red-100 text-red-600 rounded-lg text-center">{{ session('error') }}
                    </div>
                @endif
                <div class="flex gap-2 pt-3 border-t border-slate-50 dark:border-slate-800">
                    <form method="POST" action="{{ route('admin.users.suspend', $user->id) }}"
                        class="flex flex-1 items-center justify-center gap-2 py-2 px-3 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                        @csrf
                        <button type="submit">
                            <span class="material-symbols-outlined text-sm">front_hand</span> Suspend
                        </button>
                    </form>
                    <form method="POST" action="{{ route('admin.users.disable', $user->id) }}"
                        class="flex flex-1 items-center justify-center gap-2 py-2 px-3 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-xs font-bold hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
                        @csrf
                        <button type="submit">
                            <span class="material-symbols-outlined text-sm">block</span> Disable
                        </button>
                    </form>
                </div>

            </div>
        @endforeach
    </main>
    @include('admin.admin-footer')
    <div class="fixed bottom-1 left-1/2 -translate-x-1/2 w-32 h-1 bg-slate-300 dark:bg-slate-700 rounded-full z-50">
    </div>
    <script>
        document.getElementById('backBtn').addEventListener('click', function() {
            window.history.back();
        });
    </script>
</body>

</html>
